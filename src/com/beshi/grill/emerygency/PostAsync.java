...
public class PostAsync extends AsyncTask<JSONObject, Integer, String> {
	...
	private void connect_post(JSONObject obj) {
		try {
			...
			try {
				...
				if (paramArray != null) {
					...
					for (int i = 0; i < paramArray.length(); i++) {
						...
						namePair.add(new BasicNameValuePair(job
								.getString("label"), job.getString("value")));
			...
			resultString = inputStreamToString(response.getEntity()
					.getContent());

			/*
			 * if(code==200 || code==303){
			 * resultString=inputStreamToString(response
			 * .getEntity().getContent()); }
			 * 
			 * else if(code==404){ resultString = "Not found"; } else{
